...
It assumes you create the MAMP Pro site/domain manually, then let the ChatGPT desktop app in Codex mode, or another coding agent, install ProcessWire into that site's document root.
...
5. Open the site folder in the ChatGPT desktop app.
6. Choose Codex mode.
7. Send this message:
...
Codex mode should read MampWire, install ProcessWire `dev` into the current folder, create the database and dedicated database user, then return the admin credentials in chat.
...
ProcessWire database user: same as the generated database name
ProcessWire database password: generated
...
The bootstrap MySQL user is only used to create the database and dedicated site user. ProcessWire itself is installed with the dedicated database user.
...
4. Open the site folder in the ChatGPT desktop app.
5. Choose Codex mode.
6. Send only this repository URL:

```txt
https://github.com/mxmsmnv/MampWire
```

Codex mode should clone/read MampWire, run the installer against the current folder, infer the domain from MAMP Pro, and return the generated credentials in chat.
